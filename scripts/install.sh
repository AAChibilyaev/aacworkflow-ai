#!/usr/bin/env bash
# AACWorkflow installer — installs the CLI and optionally provisions a self-host server.
#
# Install / upgrade CLI only:
#   curl -fsSL https://raw.githubusercontent.com/AAChibilyaev/aacworkflow-ai/main/scripts/install.sh | bash
#
# Install CLI + provision self-host server:
#   curl -fsSL https://raw.githubusercontent.com/AAChibilyaev/aacworkflow-ai/main/scripts/install.sh | bash -s -- --with-server
#
# After installation, run `aacworkflow setup` to configure your environment.
#
set -euo pipefail
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/AAChibilyaev/aacworkflow-ai/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). AACWorkflow supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing AACWorkflow CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap AAChibilyaev/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "AACWorkflow CLI already installed via Homebrew"
    else
      warn "Failed to install aacworkflow via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "AACWorkflow CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing AACWorkflow CLI from GitHub Releases..."
  ...
  local url="https://github.com/AAChibilyaev/aacworkflow-ai/releases/download/${latest}/aacworkflow-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/aacworkflow.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/aacworkflow.tar.gz" -C "$tmp_dir" aacworkflow

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with AACWORKFLOW_BIN_DIR.
  local bin_dir="${AACWORKFLOW_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/aacworkflow" "$bin_dir/aacworkflow"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/aacworkflow" "$bin_dir/aacworkflow"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/aacworkflow" "$bin_dir/aacworkflow"
    chmod +x "$bin_dir/aacworkflow"
  ...
  rm -rf "$tmp_dir"
  ok "AACWorkflow CLI installed to $bin_dir/aacworkflow"
}
...
add_to_path() {
  ...
  for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [ -f "$rc" ] && ! grep -qF "$dir" "$rc"; then
      printf '\n# Added by AACWorkflow installer\n%s\n' "$line" >> "$rc"
    fi
  done
}
...
get_selfhost_ref() {
  if [ -n "${AACWORKFLOW_SELFHOST_REF:-}" ]; then
    printf '%s' "$AACWORKFLOW_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading AACWorkflow CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "AACWorkflow CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "AACWorkflow CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists aacworkflow; then
    local current_ver
    # `aacworkflow version` outputs "aacworkflow v0.1.13 (commit: abc1234)" — extract just the version
    current_ver=$(aacworkflow version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "AACWorkflow CLI is up to date ($current_ver)"
      return 0
    fi

    info "AACWorkflow CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(aacworkflow version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ok "AACWorkflow CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists aacworkflow; then
    fail "CLI installed but 'aacworkflow' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. AACWorkflow self-hosting requires Docker and Docker Compose.
...
# ---------------------------------------------------------------------------
setup_server() {
  info "Setting up AACWorkflow server..."
  local server_ref
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning AACWorkflow repository..."
  ...
  info "Pulling official AACWorkflow images..."
  pull_official_selfhost_images
  info "Starting AACWorkflow services (this may take a few minutes on first run)..."
  ...
  if [ "$ready" = true ]; then
    ok "AACWorkflow server is running"
  else
...
run_default() {
  printf "\n"
  printf "${BOLD}  AACWorkflow — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ AACWorkflow CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}aacworkflow setup${RESET}                # Connect to AACWorkflow Cloud (aacworkflow.com)\n"
  printf "     ${CYAN}aacworkflow setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/AAChibilyaev/aacworkflow-ai/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  AACWorkflow — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ AACWorkflow server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}aacworkflow setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/AAChibilyaev/aacworkflow-ai/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  printf "\n"
  info "Stopping AACWorkflow services..."
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No AACWorkflow installation found at $INSTALL_DIR"
  fi

  if command_exists aacworkflow; then
    aacworkflow daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the AACWorkflow CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  AACWORKFLOW_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.aacworkflow/server)"
        echo "  AACWORKFLOW_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  AACWORKFLOW_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'aacworkflow setup' to configure your environment."
        exit 0
        ;;
